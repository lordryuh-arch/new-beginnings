...
function IdlePage() {
  ...
  const ORB_TRADES: { orbId: OrbForgeId; label: string; rarity: Rarity; count: number; color: string; img: any; desc: string; baseSuccess: number; upgradeTo?: OrbForgeId; requires?: { itemId: string; qty: number; label: string } }[] = [
        ...
        >
          ...
          <div style={{
            ...
            {(() => {
              ...
              const now = Date.now();
              const buffs: Array<{
                key: string; img: any; label: string; timeMs: number;
          ...
          <div style={{
            ...
            {/* Animação de Drop de Ouro (Baúzinho) */}
            {goldDrops.map((drop) => (
              <div
                key={drop.id}
                style={{
              ...
              >
                <div style={{
                  ...
                  <img src={assetUrlFromJson(chestClosedImg)} alt="" style={{ width: 24, height: 24, imageRendering: "pixelated" }} />
                  <span style={{ color: "#ffd94d", fontSize: 12, fontWeight: 900, textShadow: "1px 1px 0 #000" }}>
                    +{drop.amount}
                  </span>
                </div>
              </div>
      ...
      {worldMapOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ position: "absolute", top: 16, left: 16, color: "#f5cf6b", fontWeight: 900, fontSize: 20, textShadow: "2px 2px 0 #000" }}>
              MAPA MUNDI 2D
            </div>
            
            <button 
              onClick={() => setWorldMapOpen(false)}
              style={{ position: "absolute", top: 16, right: 16, background: "#f5cf6b", border: "none", borderRadius: 8, padding: "4px 12px", fontWeight: 900, cursor: "pointer" }}
            >
              FECHAR
            </button>

            {/* Regiões e Marcadores (Exemplo Simples para Protótipo) */}
            <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
              {Object.entries(IDLE_MAPS).filter(([_, m]) => !m.raid).map(([id, m], i) => {
                const isUnlocked = trainerLevel >= m.minLevel;
                // Posicionamento pseudo-aleatório baseado no ID para visual
                ...
                return (
                  <button
                    ...
                    disabled={!isUnlocked}
                    onClick={() => {
                      setIdle(prev => ({ ...prev, currentMap: id as IdleMapId }));
                    ...
                    onMouseEnter={(e) => isUnlocked && (e.currentTarget.style.transform = "scale(1.2)")}
                    onMouseLeave={(e) => isUnlocked && (e.currentTarget.style.transform = "scale(1)")}
                  >
                    <div style={{ 
                      width: 24, height: 24, 
                      background: isUnlocked ? "#f5cf6b" : "#6a5a7c", 
                      borderRadius: "50%", 
                      border: "2px solid #fff",
                      boxShadow: isUnlocked ? "0 0 10px #f5cf6b" : "none"
                    }} />
                    <span style={{ 
                      color: "#fff", fontSize: 10, fontWeight: 900, 
                      textShadow: "1px 1px 0 #000", background: "rgba(0,0,0,0.6)", 
                      padding: "2px 6px", borderRadius: 4, whiteSpace: "nowrap" 
                    }}>
                      {m.name} {isUnlocked ? "" : `(Lv.${m.minLevel})`}
                    </span>
                  </button>
                );
              })}
